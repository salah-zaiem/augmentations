...
def get_files (files, n) : 
    ...
    classes = list(set([x.split("_")[1] for x in files]))
    print(f"number of clases : {len(classes)}")
    sampled_files= []
    for c in classes : 
        candidates = [ x for x in files if x.split("_")[1]==c]
...
def augment_randomly(params_file,dataset,views_number, output_dir, sampling_rate):
...
if __name__=="__main__" : 
    ...
    views_number=200
    augment_randomly(params_file, dataset, views_number, output_dir, sampling_rate)
