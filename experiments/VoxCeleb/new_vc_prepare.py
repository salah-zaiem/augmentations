...
def prepare_voxceleb(
    ...
    wav_lst_train, wav_lst_dev, wav_lst_test = _get_utt_split_lists(data_folder,iden_file
    )
    ...
    if "test" in splits:
        ...
        prepare_csv(seg_dur, wav_lst_test, save_csv_test, random_segment, amp_th)
...
def _get_utt_split_lists(
    ...
    dev_lst = []
    test_lst = []
    fileopen = open(iden_file, "r")
    lines = fileopen.read().splitlines()
    for line in lines : 
        split , path = line.split()
        if split=="1":
            train_lst.append(os.path.join(data_folders, path))
        elif split=="2":
            dev_lst.append(os.path.join(data_folders, path))
        else : 
            test_lst.append(os.path.join(data_folders, path))
    fileopen.close()
...
def prepare_csv(seg_dur, wav_lst, csv_file, random_segment=True, amp_th=0):
    """
